...
public class Fragment_Request_Open extends Fragment implements Observer, DataHolder {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_fragment__my_request__open, container, false);
        ...
        if (o instanceof ObjectTicketList) {
            if (objectTicketListRequest.getTicketListResponse().getSuccess()) {
                ...
                for (int i = 0; i < objectTicketListRequest.getTicketListResponse().getData().size(); i++) {
                    if (objectTicketListRequest.getTicketListResponse().getData().get(i).getTicketStatus().equals("0")) {
